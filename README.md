...
6. 动画与渲染优化
   ...
   - 动画结束后先提交最终棋盘 UI，再短延迟 reset 16 个格子的 `translateX/translateY`，避免提前 reset 导致方块回弹或错位。
7. 模板与节点优化
...
4. 不能只 reset “动过的格子”。真机上可能出现 transform 残留，下一次滑动会从错误位置开始。当前实现每次动画结束都会 reset 全部 16 个格子。
5. transform reset 不能放在最终棋盘 UI 提交之前。真机上如果先 reset，再更新最终方块文本和样式，可能出现方块回弹、错位或类似消失的观感。当前实现会先提交最终棋盘 UI，再短延迟 reset transform。
6. `startGroup` 仍优先于逐个 `fromTo`，因为它能减少 JS 到系统动画接口的调用次数；但参数对象必须用安全快照。
7. 如果后续设备或系统版本上 `startGroup` 再出现兼容问题，可以退回逐个 `folme.fromTo({ fromState, toState })` 的保守实现。
